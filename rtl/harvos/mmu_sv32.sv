...
`ifndef SYNTHESIS
  ...
  generate
    for (gi=0; gi<TLB_ENTRIES; gi=gi+1) begin: G_INV
      ...
      assert property (!(itlb_p_w[gi] && itlb_p_x[gi])) else $error("W^X violated in ITLB at %0d", gi);
      assert property (!(dtlb_p_w[gi] && dtlb_p_x[gi])) else $error("W^X violated in DTLB at %0d", gi);
    end
  endgenerate
`endif
