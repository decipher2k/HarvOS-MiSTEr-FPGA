...
(
	...
	localparam int DMA_WINDOW_BYTES = 64;
	...
	initial begin
		...
		imem[31] = enc_i(12'sd1,  5'd7, F3_ADD_SUB, 5'd7, OPC_OP_IMM);
		imem[32] = enc_s(12'sd8,  5'd7, 5'd1, F3_LW_SW);
		imem[33] = enc_j(-21'sd8, 5'd0);
	...
	assign dma_req = (dma_tick[9:0] == 10'h3FF);
	assign dma_addr = DMA_WINDOW_BASE + {26'h0000000, dma_tick[5:2], 2'b00};
	assign dma_status = {30'h0000_0000, dma_fault, dma_allow};
	assign trace_event = bus_req || dma_req || debug_trap0 || debug_trap1 || kernel_policy_fault;
	...
	always_ff @(posedge clk) begin
		if(reset) begin
			...
			if(dma_req && dma_allow) begin
				dmem[dma_index] <= entropy_lfsr ^ {16'hD00D, dma_tick};
			end
		end
	end
	...
	harvos_trace trace (
		...
		.event_data(bus_req ? bus_addr : (dma_req ? dma_addr : debug_scause)),
		.digest(trace_digest),
		.count(trace_count)
	);
