...
(
	...
	input  logic [31:0] pt_l1_rdata,
	input  logic [31:0] pt_l0_rdata,

	output logic [31:0] pt_l1_addr,
	output logic [31:0] pt_l0_addr,
	...
	harvos_pagetable #(
		...
		.vaddr(vaddr),
		.satp(satp),
		.l1_pte_rdata(pt_l1_rdata),
		.l0_pte_rdata(pt_l0_rdata),
		.l1_index(l1_index),
		.l0_index(l0_index),
		.l1_pte_addr(pt_l1_addr),
		.l0_pte_addr(pt_l0_addr),
		.paddr(walk_paddr),
		.flags(walk_flags),
		.valid(walk_valid),
		.cause(walk_cause)
	...
	always_comb begin
		...
		// bit 4 is "MMIO" to match the HarvOS ABI capability mask. Supervisor
		// bootstrap starts with all caps clear but is exempt; user accesses are
		// constrained.
		if(user_mode && allow) begin
			if(in_mmio) allow = caps[CAP_MMIO_BIT];
			else allow = caps[CAP_NORMAL_MEMORY_BIT];
		end
