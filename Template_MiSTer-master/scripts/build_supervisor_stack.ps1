...
param(
  ...
  [string]$OutDir = "build/supervisor"
)
...
$Objects = @()
Get-ChildItem (Join-Path $Root "software/supervisor/src") -Filter "*.c" | Sort-Object Name | ForEach-Object {
...
$Archive = Join-Path $OutPath "libharvos_supervisor.a"
if (Test-Path $Archive) { Remove-Item $Archive }
& $Ar rcs $Archive @Objects
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

if (Test-Path $Objdump) {
  $Disasm = Join-Path $OutPath "harvos_supervisor.dis"
  "" | Set-Content $Disasm
...
Write-Output "supervisor_archive=$Archive objects=$($Objects.Count)"
